...
func NewPackageTask(cfg cataloging.Config, c pkg.Cataloger, tags ...string) Task {
	fn := func(resolver file.Resolver, sbom SBOMBuilder) error {
		...
		log.WithFields("cataloger", c.Name()).Debugf("discovered %d packages", pkgs)

		for i, p := range pkgs {
			if cfg.DataGeneration.GenerateCPEs {
			...
			if cfg.DataGeneration.GuessLanguageFromPURL {
